class UsersController < ApplicationController
  ...
  def show
    if current_user.admin
      ...
      @first_round = Round.where(name: "first")[0]
      @second_round = Round.where(name: "second")[0]
      @final_round = Round.where(name: "final")[0]
      # binding.pry
      @first_round_pitches = Pitch.where(round_id: @first_round.id)
      @second_round_pitches = Pitch.where(round_id: @second_round.id) if @second_round
      @final_pitches = Pitch.where(round_id: @final_round.id) if @final_round
